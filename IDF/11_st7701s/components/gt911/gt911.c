...
void gt911_write_reg(uint16_t reg, uint8_t *data, uint8_t len)
{
    ...
    i2c_master_write_to_device(I2C_NUM_0, GT911_ADDR, send, len + 2, 1000 / portTICK);
...
void gt911_read_reg(uint16_t reg, uint8_t *data, uint8_t len)
{
    ...
    i2c_master_write_read_device(I2C_NUM_0, GT911_ADDR, send, 2, data, len, 1000 / portTICK);
}
...
void gt911_start(void)
{
    xTaskCreate(gt911_task_scan, "gt911", 1024 * 3, NULL, 3, NULL);
}
