...
void switch_event_cb(lv_event_t *e)
{
    ...
    if (target == switch0)
    {
        if (status)
        {
            ...
            esp_mqtt_client_publish(mqttdev.client, mqttRoomLedCtrl, "{\"status\":\"on\"}", 0, 0, 0);
            lv_label_set_text_fmt(label00, "ON");
        }
        else
        {
            esp_mqtt_client_publish(mqttdev.client, mqttRoomLedCtrl, "{\"status\":\"off\"}", 0, 0, 0);
            lv_label_set_text_fmt(label00, "OFF");
        }
    }
...
void timer_cb(lv_timer_t *t)
{
    ...
    lv_label_set_text_fmt(my_time.label, "%02d:%02d", date.shi, date.fen);
    lv_label_set_text_fmt(timelabel, "%02d:%02d:%02d", date.shi, date.fen, date.miao);
}
